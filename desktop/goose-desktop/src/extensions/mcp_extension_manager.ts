...
export const createExtensionManager = () => {
	 ...
	 */
	const registerExtension = async (config: ExtensionConfig): Promise<RegistrationResult> => {
	...
	const executeTool = async (
		...
		if (!extensionTools) {
			return createToolError(ErrorCode.EXTENSION_NOT_FOUND, `Extension ${extensionName} not found`);
		}
	 ...
	 */
	const getToolInfo = (extensionName: string, toolName: string): Tool | undefined => {
...
// Create a singleton instance of the extension manager
export const extensionManager = createExtensionManager();
